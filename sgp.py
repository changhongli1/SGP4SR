...
class SGP(SequentialRecommender):
    ...
    def process_tensors(self, x, y):
        m, n = x.shape
        x_part = x[:, :self.knn_k_ma]
        y_part = y[:, :self.knn_k_co]
        z = torch.cat((x_part, y_part), dim=1)
        def remove_duplicates(tensor):
            ...
            for row in tensor:
                ...
                seen = set()
                for val in row:
                    if val.item() not in seen:
                        unique_row.append(val)
                        seen.add(val.item())
                unique_rows.append(unique_row)
            return unique_rows
        z_unique_rows = remove_duplicates(z)
        def ensure_fixed_length(unique_rows, total_length):
            result = []
            for row in unique_rows:
                missing_len = total_length - len(row)
                if missing_len > 0:
                    result.append(row + row[:missing_len]) 
                else:
                    result.append(row)
            return torch.tensor(result)
        z_unique = ensure_fixed_length(z_unique_rows, n)
        num_missing = n - z_unique.shape[1]
        x_remaining = x[:, self.knn_k_ma:]
        y_remaining = y[:, self.knn_k_co:]
        x_to_add = x_remaining[:, :num_missing//2]
        y_to_add = y_remaining[:, :num_missing - num_missing//2]
        z_final = torch.cat((z_unique.cuda(), x_to_add.cuda(), y_to_add.cuda()), dim=1)

        return z_final
    
    def select_nei(self, m_kk, co_kk):
        def replace_elements(q, a, z):
            m, n = z.shape
            for i in range(m):
                q_row = q[i]
                a_row = a[i]
                z_row = z[i]
                to_replace_idx = (z_row == -1).nonzero(as_tuple=True)[0]
                valid_q_elements = [q_row[j].item() for j in range(n) if q_row[j] != -1]
                valid_a_elements = [a_row[j].item() for j in range(n) if a_row[j] != -1]
                replace_elements = []
                idx_q, idx_a = 0, 0
                while len(replace_elements) < len(to_replace_idx):
                    if idx_q + self.d <= len(valid_q_elements):
                        replace_elements.extend(valid_q_elements[idx_q:idx_q + self.d])
                        idx_q += self.d
                    if idx_a < len(valid_a_elements):
                        replace_elements.append(valid_a_elements[idx_a])
                        idx_a += 1
                for idx, replacement in zip(to_replace_idx, replace_elements):
                    z_row[idx] = replacement
        ...
        for i in range(m):
            common_elements = set(x[i].tolist()) & set(y[i].tolist())
            
            p_idx = 0  
            for elem in common_elements:
                x_idx = (x[i] == elem).nonzero(as_tuple=True)[0]
                y_idx = (y[i] == elem).nonzero(as_tuple=True)[0]
                p[i, p_idx] = elem
                p_idx += 1
                x[i, x_idx] = -1
                y[i, y_idx] = -1
        return replace_elements(x, y, p)
